    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int power = 1,
                ...
                leftEnd, rightEnd,
                endAll = 0;
            //int[] arr = { 36, 10, 1, 34, 28, 38, 31, 27, 30, 20, 21,22 };
            //int[] arr = { 3, 4, 1, 5, 2, 6 };
            int[] arr2 = new int[n];
            ...
            while (endAll == 0)
            {
                for (int i = 0; i < n; i += (int)Math.Pow(2, power) * 2)
                {
                    ...
                    if ((int)Math.Pow(2, power) * 2 > n)
                    {
                        ...
                        int rotations = rightEnd - indexOfRightSmallest;
                        //tuka ne e po uslovie
                        for (int j = 0; j < n - rotations; j++)
                        {
                            indexOfRightSmallest = leftEnd;
                            indexOfLeftSmallest = j;
                            for (int h = 0; h < rotations; h++)
                            {

                                if (arr2[indexOfRightSmallest] < arr2[indexOfLeftSmallest])
                                {
                                    smallest = arr2[indexOfRightSmallest];
                                    arr2[indexOfRightSmallest] = arr2[indexOfLeftSmallest];
                                    arr2[indexOfLeftSmallest] = smallest;
                                }
                        ...
                        indexOfRightSmallest = leftEnd;
                        //tuka sushto
                        for (int r = indexOfRightSmallest; r < n; r++)
                        {
                            smallest = arr2[r];
                            for (int f = r; f < n; f++)
                            {
